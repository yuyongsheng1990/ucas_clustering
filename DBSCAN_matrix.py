...
def co_exist(upc_list):
    ...
    for i in upc_list:
        upc1=i.strip() #分类号'H04L12/24'
        upc1_num=upc_list_all.index(upc1) #分类号离散化，用数字0表示
        upc_list_copy.pop(0)
        for j in upc_list_copy:
            upc2=j.strip()
            upc2_num=upc_list_all.index(upc2)
            if upc1_num in row and upc2_num in col: #coo_matrix的行和列-列表中是否包含upc分类号
                temp_num=0
                foo_value=True
                while temp_num < len(row): #遍历；判断行分类号在row中的index与列分类号在col中的index是否对齐，若对齐则为同一共现，否则不为同一共现
                    #如共现(2,3)，而现有的row=[2,4];col=[1,3]
...
# 1.读入excel英文专利数据
table=file_input.sheets()[1] #sheet1存储着原始数据2015-2017年的数据
    #1.1将所有不同的技术类号添加到列表中
upc_list_all=[]
row=[]
col=[]
...
file_output.close()'''
#3. 直接在python内部聚类
x_result=db(eps=4.5,min_samples=2).fit_predict(co_exist_matrix)
print(x_result)
for i in x_result:
